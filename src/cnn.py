...
import time
import sys
from cnn_core import CnnCore
import cnn_data_engine as de
...
if __name__ == '__main__':
  ...
  x_imgs = tf.reshape(x, [-1, HEIGHT, WIDTH, CHANNEL])
  y_ = tf.placeholder(tf.float32, shape=[None, NUM_CLASSES])

  # Specify nn architecture
  model = CnnCore(dataset_name, CHANNEL, HEIGHT, NUM_CLASSES, TRAIN_BATCH_SIZE)
  y = model.inference(x_imgs)
  ...
  with tf.Session() as sess:
    ...
    for i in range(EPOCH):
      ...
      if i % 100 == 0:
        ...
        train_log = sess.run([loss, accuracy, batch_correct_predict, batch_size], feed_dict={x: batch[0], y_: batch[1]})
        train_loss = train_log[0]
        train_acc = train_log[1]
        train_correct_predict = train_log[2]
        train_batch_size = train_log[3]
